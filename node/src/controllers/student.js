...
// DELETE - remove a student from the database using id or email
export const removeStudent = async (req, res) => {
  console.log("DELETE - client wants to remove a student...")
  let param = req.params.idOrEmail
  if (param == '')
    return res.status(400).send('Parameter field is empty. Cannot process request.')
  let query = ''
  if (isNumeric(param))
    query = `SELECT removeStudentById(${param}) AS student`
  else
    query = `SELECT removeStudentByEmail('${param}') AS student`
  console.log("Removing student from database...")
  //let query = `SELECT removeStudentById('${id}') AS student`
  try {
  const results = await db.query(query)
    console.log("Sending response back to client...")
    let student = results.rows[0]["student"]
    if (student["id"] == null) {
      return res.status(404).send('Cannot remove student. No student is registered with that id or email.')
    }
    return res.status(200).json({
      id: student["id"],
      fname: student["first_name"],
